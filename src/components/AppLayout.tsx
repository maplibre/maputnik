...
const getDefaultSidebarPercent = () => {
  const viewportWidth = typeof window !== "undefined" ? window.innerWidth : DEFAULT_SIDEBAR_WIDTH;
  return Math.min(100, (DEFAULT_SIDEBAR_WIDTH / (viewportWidth || DEFAULT_SIDEBAR_WIDTH)) * 100);
};
      ...
      >
        ...
        <div className="maputnik-layout-main">
          ...
          >
            ...
            >
              {props.codeEditor ? (
                <ScrollContainer>{props.codeEditor}</ScrollContainer>
              ) : (
                <Group
            ...
            />
            <Panel id={MAP_PANEL_ID} className="maputnik-layout-map" minSize={0}>
              {props.map}
            </Panel>
          </Group>
        </div>
        {props.bottom && <div className="maputnik-layout-bottom">{props.bottom}</div>}
        {props.modals}
      </div>
